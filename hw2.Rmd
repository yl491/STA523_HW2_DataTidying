...
```{r}
lego_sales %>% 
  mutate(quantity = as.integer(quantity), usprice = as.numeric(as.character(usprice)), receipts = quantity*usprice, full_name = paste(first_name,last_name)) %>% 
...
t2 = sapply(unique(t1), function(name){sum(t1 == name)}) %>% sort(.,decreasing = TRUE)
hobbies = t2[1:5]

#J.C. - hopefully this works
```
